...
Reference doc for how the Zorin blog is built, styled, and maintained. Last updated 2026-09-26 (Google-guidance quality pass: merges, keyword slugs, sourcing).

**Post count as of 2026-09-26: 76 total, 71 visible, 5 hidden** (noindex/off-nav; `guest-posting-strategy-for-ecommerce-stores` joined the hidden set 2026-09-26). `src/lib/blog/posts.ts` is the only authoritative source; the "Current post list" table below is a historical snapshot only.

**2026-09-26 quality pass (after Google's March/May 2026 core updates, Aug/Sep 2026 spam updates and the May 2026 AI-features guide):**
- **Merged 10 cannibalizing posts into 5 keepers**, each with a 308 in `next.config.ts` (`BLOG_REDIRECTS` table: keep destinations pointing at live slugs, never chain). Keeper chosen from GSC ranking history. `canonicalSlug` is no longer used by any post; retire a duplicate with a redirect instead.
- **Renamed 9 low-GSC-history slugs to keyword slugs** (e.g. `what-is-dynamic-pricing`, `price-elasticity-of-demand-formula`, `how-to-raise-prices-without-losing-customers`, `competitor-price-tracking`, `price-optimization-tools`, `pricing-software-for-small-business`, `dynamic-pricing-software`, `automated-pricing-shopify`, `competitive-pricing-strategy`). Posts with real GSC traction kept their slugs; only titles/meta changed.
- **Every visible post now cites at least one verified external source.** Verified stats and cases (with the correct figures and dead-URL notes) are in the auto-memory `zorin-verified-source-bank`. Notable corrections: McKinsey's figure is ~8% for the S&P 1500 (not 11%, Global 1200); the Eras Tour was not a dynamic-pricing case (Springsteen 2022 is); Intelligems price testing is $1,199/mo module pricing, not $499.
- **No first-person experience claims that can't be verified** (removed two). **No branded "Where Zorin Fits" H2s** in non-comparison posts; keep one short product paragraph instead.
- Key Takeaways moved to the closing-recap position (before FAQ) in 37 older posts that still had it after the intro.

**Real-world-example requirement (added 2026-09-15):** every post now needs one real, named, verifiable real-world example (recent news, a company's documented decision, a legal case) in addition to statistics research, not instead of it — modeled on the Springsteen/Wendy's cases in `what-is-dynamic-pricing` (the earlier Ticketmaster/Taylor Swift framing was inaccurate: Eras Tour face prices were capped and Swift declined dynamic pricing). A missing example is explicitly preferred over a fabricated or weakly-fitting one. Full rule in `zorin-blog-content-pipeline.md`.
...
| 2026-08-16 | `competitor-price-tracking` (formerly `do-you-need-a-competitor-price-tracking-app`) | Product |
