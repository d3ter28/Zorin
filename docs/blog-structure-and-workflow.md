...
Reference doc for how the Zorin blog is built, styled, and maintained. Last updated 2026-09-25 (post count, MOFU loop 2 complete).

**Post count as of 2026-09-25: 86 total** (was 47 at the 2026-08-21 architecture update, was pinned in this doc's stale "Current post list" table below at 47 — that table is a historical snapshot only, `src/lib/blog/posts.ts` is the only authoritative source). 12 MOFU (loop 2 now fully complete across all 6 pillars), 6 BOFU (4 comparison layout, 2 alternatives format, owned by the separate `zorin-comparison-writer` skill), 4 hidden (noindex/off-nav guest-exchange posts, excluded from all counts). Full detail on everything added since 2026-09-14, including a same-day post swap (2 SaaS-pricing posts removed and replaced with 2 bergerCMO.ai guest posts), the linkable-asset backlog nearly clearing (glossary, price-increase email template, markdown/clearance spreadsheet, and discount-depth-by-category research page all shipped 2026-09-20), a Screaming Frog crawl + GSC/Bing performance review with fixes (2026-09-20/21), and the 2 MOFU posts closing out loop 2 (2026-09-22/25), lives in the `Zorin Blog Handover.md` and `Zorin SEO Handover.md` Obsidian docs, not repeated here.

**Real-world-example requirement (added 2026-09-15):** every post now needs one real, named, verifiable real-world example (recent news, a company's documented decision, a legal case) in addition to statistics research, not instead of it — modeled on the Ticketmaster/Taylor Swift case in `dynamic-pricing-explained-definition-example-legal-risks`. A missing example is explicitly preferred over a fabricated or weakly-fitting one. Full rule in `zorin-blog-content-pipeline.md`.
