...
Reference doc for how the Zorin blog is built, styled, and maintained. Last updated 2026-09-20 (post count, linkable-asset backlog nearly cleared, real-world-example skill rule).

**Post count as of 2026-09-20: 84 total** (was 47 at the 2026-08-21 architecture update, was pinned in this doc's stale "Current post list" table below at 47 — that table is a historical snapshot only, `src/lib/blog/posts.ts` is the only authoritative source). 10 MOFU, 6 BOFU (4 comparison layout, 2 alternatives format, owned by the separate `zorin-comparison-writer` skill), 4 hidden (noindex/off-nav guest-exchange posts, excluded from all counts). Full detail on everything added since 2026-09-14, including a same-day post swap (2 SaaS-pricing posts removed and replaced with 2 bergerCMO.ai guest posts) and the linkable-asset backlog nearly clearing (glossary, price-increase email template, markdown/clearance spreadsheet, and discount-depth-by-category research page all shipped 2026-09-20), lives in the `Zorin Blog Handover.md` Obsidian doc, not repeated here.
