...
interface PaymentFeeCalculatorProps {
  presets: Record<string, FeePreset>;
...
export function PaymentFeeCalculator({
  ...
  const [preset, setPreset] = useState(defaultPresetKey);
  const [feePct, setFeePct] = useState(presets[defaultPresetKey]?.feePct ?? "");
  const [feeFixed, setFeeFixed] = useState(presets[defaultPresetKey]?.feeFixed ?? "");
  ...
  return (
    <div className="rounded-xl border border-zinc-200 bg-white p-8 shadow-sm">
      ...
      <div className="mt-8 border-t border-zinc-100 pt-6">
        ...
        <p className="mt-1 text-xs text-zinc-500">{selectorHelp}</p>
        <div className="mt-4 grid gap-5 sm:grid-cols-3">
          <div className="flex flex-col gap-1.5">
            <label htmlFor="preset" className="text-sm font-medium text-zinc-700">
              {selectorLabel}
            </label>
            <select
              id="preset"
